...
class TestHand:
    ...
    def test_discard(self):
        ...
        hand_player_cards = hand_player.cards

        discard_tray.discard(hand_player.discard())

        assert len(deck) == initial_size - 3
        assert len(discard_tray) == 3
        assert hand_player_cards in discard_tray._discard_deck
...
class TestHandPlayer:
    def test_hand(self):
        ...
        assert hand_player.cards == cards
    ...
    def test_split(self):
        ...
        assert Card(suit="Spades", rank="10") in hand_1.cards
        assert Card(suit="Hearts", rank="Jack") in hand_2.cards
        assert hand_2.active == False
...
class TestHandDealer:
    def test_hand(self):
        ...
        assert hand_dealer.cards == [Card(suit="Spades", rank="Jack"), "hidden"]
